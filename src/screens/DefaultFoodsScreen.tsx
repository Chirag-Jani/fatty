...
import { addFoodTemplate, deleteFoodTemplate, getFoodTemplates, updateFoodTemplate } from '../storage/storage';
...
export function DefaultFoodsScreen() {
  ...
  async function save() {
    ...
    if (!name) return;
    if ([kcalPer100, proteinPer100, carbsPer100, fatPer100].some((n) => Number.isNaN(n))) return;
    ...
    const payload: FoodTemplate = {
      ...
      carbsPer100,
      fatPer100,
      createdAt: editingId ? templatesById[editingId]?.createdAt ?? Date.now() : Date.now(),
    };
        ...
        {templates.length === 0 ? (
          <Card>
            ...
            <Text style={styles.emptySub}>
              Add your most common foods and drinks (grams or ml). Then log them from the Food tab.
            </Text>
            <View style={{ height: SPACING.md }} />
            <PrimaryButton title="Create first default food" onPress={openCreate} />
          </Card>
        ) : (
          templates.map((t) => (
            <Card key={t.id}>
              <View style={styles.rowTop}>
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.meta}>
                    {Math.round(t.kcalPer100)} kcal / 100{t.amountUnit} · P {t.proteinPer100} · C {t.carbsPer100} · F{' '}
                    {t.fatPer100}
                  </Text>
                </View>
                <View style={styles.actions}>
                  <Pressable style={styles.iconBtn} onPress={() => openEdit(t)} hitSlop={10}>
                    <Ionicons name="create-outline" size={18} color={COLORS.primary} />
                  </Pressable>
                  <Pressable style={styles.iconBtnDanger} onPress={() => remove(t.id)} hitSlop={10}>
        ...
        >
          ...
          >
            <View style={styles.sheetTop}>
              <Text style={styles.sheetTitle}>{mode === 'edit' ? 'Edit default food' : 'New default food'}</Text>
            ...
            <ScrollView keyboardShouldPersistTaps="handled">
              ...
              <Text style={styles.label}>Calories per 100{form.amountUnit} (kcal)</Text>
              <TextInput
                style={styles.input}
              ...
              <View style={styles.grid}>
                <View style={styles.gridCol}>
                  <Text style={styles.label}>Protein / 100{form.amountUnit}</Text>
                  <TextInput
                    style={styles.input}
                    value={form.proteinPer100}
                    onChangeText={(t) => setForm((f) => ({ ...f, proteinPer100: t }))}
                    keyboardType="decimal-pad"
                    placeholder="g"
                    placeholderTextColor={COLORS.textSecondary}
                  />
                ...
                <View style={styles.gridCol}>
                  ...
                  <TextInput
                    ...
                    onChangeText={(t) => setForm((f) => ({ ...f, carbsPer100: t }))}
                    keyboardType="decimal-pad"
                    placeholder="g"
                    placeholderTextColor={COLORS.textSecondary}
                  />
              ...
              <PrimaryButton title={mode === 'edit' ? 'Save changes' : 'Save default food'} onPress={save} />
...
const styles = StyleSheet.create({
  ...
  meta: { marginTop: 4, fontSize: 12, color: COLORS.textSecondary, lineHeight: 18 },
  ...
  chipOn: { backgroundColor: COLORS.chipSelectedBg, borderColor: COLORS.primaryDark },
